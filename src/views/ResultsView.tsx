...
import { Upload, Download, Trash2, Plus, BarChart3, ListFilter, CheckCircle2, XCircle, AlertCircle, Search, HelpCircle, X, Printer, FileText, ChevronDown, Award, TrendingUp, UserCheck, Layers, Calendar, CheckSquare, Square, Bell } from 'lucide-react';
import * as XLSX from 'xlsx';
import { StudentProgressCharts } from '../components/StudentProgressCharts';

export const ResultsView = () => {
  const { state, setResults, setStudents, userRole, openNotificationModal } = useAppContext();
  ...
  return (
    <div className="space-y-2.5 sm:space-y-6 md:space-y-8 flex flex-col h-full relative font-sans text-brand-ink">
        ...
        <div className="w-full sm:w-auto">
          <div className="flex items-center justify-between sm:justify-start gap-2">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <h2 className="text-xl sm:text-3xl md:text-4xl font-serif text-brand-ink font-bold tracking-tight leading-tight">Sınav Sonuçları</h2>
            </div>
            <span className="sm:hidden text-[11px] font-bold text-brand-ink/70 bg-[#F5F4F0] px-2 py-0.5 rounded-md border border-brand-border">
              {summaryStats.totalCount} Öğrenci
            </span>
          </div>
          <p className="hidden sm:block text-brand-ink/60 text-xs sm:text-sm mt-1">Öğrenci ders netleri, yanlış-doğru-boş takibi ve karne analizi</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto py-0.5">
          {selectedResultIds.length > 0 && userRole === 'admin' && (
            <button 
              ...
              className="flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-rose-600 text-white rounded-xl text-xs font-bold shadow-xs hover:bg-rose-700 active:scale-95 transition-all min-w-0 w-full sm:w-auto cursor-pointer"
            >
          ...
          {userRole === 'admin' && (
            <>
              ...
              <button 
                ...
                className="flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-white border border-brand-border text-xs font-bold text-brand-ink rounded-xl transition-all hover:bg-[#FAF9F6] active:scale-95 shadow-2xs sm:shadow-xs cursor-pointer min-w-0 w-full sm:w-auto"
              ...
              <button 
                onClick={() => {
                  const examTitle = selectedExam || 'Deneme Sınavı';
                  openNotificationModal({
                    type: 'exam_result',
                    title: `${examTitle} Sonuçları Açıklandı!`,
                    message: `${examTitle} deneme sınavı karneleri, ders netleri ve genel sıralamalar sisteme yüklenmiştir. Sonuçlarınızı görmek için tıklayınız.`,
                    linkTab: 'results',
                    targetGrade: selectedGrade === 'all' ? 'Tümü' : selectedGrade,
                    urgent: false
                  });
                }} 
                className="flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-amber-500/15 to-brand-accent/20 border border-brand-accent/40 text-xs font-bold text-amber-900 dark:text-amber-300 rounded-xl transition-all hover:bg-brand-accent/30 active:scale-95 shadow-2xs sm:shadow-xs cursor-pointer min-w-0 w-full sm:w-auto"
                title="Tüm kullanıcılara tarayıcı üzerinden anlık sınav sonucu bildirimi gönder"
              >
                <Bell className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
                <span className="truncate">Sonuçları Bildir</span>
              </button>
              ...
              <button 
                ...
                className="flex items-center justify-center gap-1 sm:gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 bg-[#151618] border border-[#151618] text-white text-xs font-bold rounded-xl transition-all hover:bg-black active:scale-95 shadow-2xs sm:shadow-xs cursor-pointer min-w-0 w-full sm:w-auto"
              >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-brand-border/70 pb-2">
        <div className="flex items-center gap-1 bg-[#FAF9F6] p-1 rounded-xl w-full sm:w-auto border border-brand-border/70 shadow-2xs">
          <button
            onClick={() => setActiveTab('summary')}
            className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold rounded-lg transition-all text-center truncate cursor-pointer ${
              activeTab === 'summary'
          ...
          <button
            ...
            className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1 sm:gap-1.5 truncate cursor-pointer ${
              activeTab === 'detailed'
                ? 'bg-white text-brand-ink shadow-xs border border-brand-border/60'
                : 'text-brand-ink/60 hover:text-brand-ink hover:bg-black/5'
            }`}
          >
            <BarChart3 className="h-3.5 w-3.5 shrink-0 text-amber-600" />
