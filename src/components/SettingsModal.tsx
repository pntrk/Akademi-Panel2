...
export const SettingsModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  const handleRemoveAdmin = (email: string) => {
    if (isSuperAdmin(email)) return; // Cannot remove super admin
            ...
            <div className="space-y-2">
              {admins.map(email => (
                <div key={email} className="flex items-center justify-between p-3 bg-emerald-50/70 rounded-xl border border-emerald-100">
                  <span className="text-xs sm:text-sm font-medium text-emerald-800 truncate mr-2">{email} {isSuperAdmin(email) && '(Süper Admin)'}</span>
                  {!isSuperAdmin(email) && (
