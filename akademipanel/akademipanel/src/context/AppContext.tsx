...
export const AppProvider = ({ children, user }: { children: ReactNode, user: User }) => {
  ...
  const initialRole = (() => {
    ...
    if (email === 'kirklareliataturkortaokulu@gmail.com') return 'admin';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      ...
      const userEmail = user.email || '';
      if (userEmail === 'kirklareliataturkortaokulu@gmail.com' || stateRef.current.admins?.includes(userEmail)) {
        setUserRole('admin');
