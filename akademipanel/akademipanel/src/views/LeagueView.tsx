      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-amber-500/15 text-amber-700 flex items-center justify-center shrink-0 font-bold">
              <Trophy className="w-4 h-4" />
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif text-[#5a5a40] font-bold tracking-tight leading-tight">🏆 Akademi Arena</h2>
          </div>
          <p className="text-brand-ink/60 text-xs sm:text-sm mt-0.5">Takımların başarı grafikleri, lig puanları ve rozet sistemi</p>
        </div>

        <button
          onClick={() => setActiveView(activeView === 'dashboard' ? 'rules' : 'dashboard')}
          className="flex items-center gap-1.5 bg-[#151618] hover:bg-black text-white px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-xl transition-all shadow-sm text-xs font-bold active:scale-95 cursor-pointer shrink-0"
        >
          {activeView === 'dashboard' ? (
            <><BookOpen className="w-3.5 h-3.5 text-amber-400" /> Rozet Rehberi & Kurallar</>
          ) : (
            <><Trophy className="w-3.5 h-3.5 text-amber-400" /> Liderlik Tablosuna Dön</>
          )}
        </button>
      </header>
