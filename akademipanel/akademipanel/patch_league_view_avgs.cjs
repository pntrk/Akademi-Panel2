...
const targetStr = `  const calcAvg = (teamName: string, team: any[]) => {`;
...
// Then delete the old eighthGradeStudents definition further down
const secondTarget = `  const eighthGradeStudents = useMemo(() => {
    return state.students.filter(s => {
      const isEighthGrade = s.className?.startsWith('8');
      const hasExams = state.results.some(r => r.studentNo === s.no && s.no !== 0 && Object.keys(r.scores || {}).length > 0);
      return isEighthGrade && hasExams;
...
code = code.replace(secondTarget, '');
